...
`sh examples/make_examples.sh` from [`examples/incident.txt`](examples/incident.txt)
and the RFC's Appendix D:

| File | What it is |
|---|---|
| [`inventory.png`](examples/output/inventory.png) / `.pdf` | The 76-primitive chart: every head shape and every modifier mark in its §6.2 position |
| [`urgency.png`](examples/output/urgency.png), [`deadline.png`](examples/output/deadline.png), [`outage.png`](examples/output/outage.png), [`root-cause.png`](examples/output/root-cause.png), [`blast-radius.png`](examples/output/blast-radius.png) | Appendix A compositions drawn as script blocks (nested roles, residue ribbon), with `.txt` showing SID, canonical bytes and reading |
| [`incident-translation.png`](examples/output/incident-translation.png) / `.pdf` / [`.txt`](examples/output/incident-translation.txt) | Eight English sentences → compositions, side by side, with residue stats |
| [`incident.alpb`](examples/output/incident.alpb) → [`incident.alpt`](examples/output/incident.alpt) | The same text encoded as a 19-event ALP/B stream (SID-128) and its ALP/T projection |
| [`incident-archive-sid256.alpt`](examples/output/incident-archive-sid256.alpt) | The stream re-profiled to SID-256 for archival (§3.5) |
| [`incident-audit.pdf`](examples/output/incident-audit.pdf) / `.png` | Stream audit document: every event, its blocks, the ALP/T text |
| [`incident-decoded.txt`](examples/output/incident-decoded.txt) | ALP → English, from stored glosses and from generated readings |
| [`incident-verify.txt`](examples/output/incident-verify.txt), [`incident-stats.txt`](examples/output/incident-stats.txt) | Hash / canonical-form / round-trip check and size figures |
